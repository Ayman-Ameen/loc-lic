...
def conv_block(in_channels, out_channels, kernel_size=5, stride=2):
    return nn.Sequential(
        nn.Conv2d(in_channels, out_channels, kernel_size, stride=stride, padding=kernel_size // 2),
        nn.GELU(),
    )

def deconv_block(in_channels, out_channels, kernel_size=5, stride=2):
    return nn.Sequential(
        nn.ConvTranspose2d(in_channels, out_channels, kernel_size, stride=stride, output_padding=stride - 1, padding=kernel_size // 2),
        nn.GELU(),
    )

class residal_block(nn.Module):
    def __init__(self, channels=[3,32,64], kernel_size=3, stride=[2,4], layer_func=conv_block, double_channels=True):
        super(residal_block, self).__init__()
        assert len(channels) == 3 and len(stride) == 2, "channels and stride should be 3 and 2"
        self.channels = channels
        if double_channels:
            channels_conv1 = channels[0], channels[1]
            channels_conv2 = channels[1], channels[2]
            channels_res = channels[0], channels[1]
        else:
            channels_conv1 = channels[0], channels[1]
            channels_conv2 = channels[1], channels[2]
            channels_res = channels[0], channels[2]
        self.stride   = stride
        self.conv1    = layer_func(channels_conv1[0], channels_conv1[1], kernel_size, stride=stride[0])
        self.conv2    = layer_func(channels_conv2[0], channels_conv2[1], kernel_size, stride=stride[0])
        self.conv_res = layer_func(channels_res[0], channels_res[1], kernel_size, stride=stride[1])
        self.norm1    = nn.BatchNorm2d(channels_conv1[0])
        self.norm2    = nn.BatchNorm2d(channels_conv2[0])
        self.norm_res = nn.BatchNorm2d(channels_res[0])
        self.double_channels = double_channels

    def forward(self, x):
        res = self.conv_res(self.norm_res(x))      # x [b,3,h,w] -> res [b,32,h/4,w/4]
        if self.double_channels:
            res = torch.cat([res, res], dim=1)          # res [b,64,h/4,w/4]
        x1  = self.conv1(self.norm1(x))            # x1 [b,32,h/2,w/2]
        x2  = self.conv2(self.norm2(x1))           # x2 [b,64,h/4,w/4]
        return x2 + res
    

class LoC_LIC_residually(CompressionModel):
    def __init__(self, config, **kwargs):
        super().__init__(config.N[-1], **kwargs)
        N = config.N
        M = config.M
        self.N = N
        self.M = M

        self.entropy_bottleneck = EntropyBottleneck(M[-1])

        layers = []
        k=0
        layers.append(residal_block(channels=[3, N[k], N[k+1]]))
        layers.append(residal_block(channels=[N[k+1], N[k+2], M[0]]))
        self.g_a = nn.Sequential(*layers)
        ...
        N_s = N[::-1]
        k=0
        layers.append(residal_block(channels=[M[0], N_s[k], N_s[k+1]],layer_func=deconv_block, double_channels=False))
        layers.append(residal_block(channels=[N_s[k+1], N_s[k+2], 3],layer_func=deconv_block, double_channels=False))
        self.g_s = nn.Sequential(*layers)

        layers = []
        for k in range(len(M)):
            if k == 0:
                layers.append(conv_block(M[k], M[k], stride=1))
            else:
                layers.append(conv_block(M[k-1], M[k]))

        self.h_a = nn.Sequential(*layers)

        layers = []
        M_s= M[::-1]
        for k in range(len(M_s)):
            if k == len(M_s) - 1:
                layers.append(deconv_block(M_s[k], M_s[k], stride=1))
            else:
                layers.append(deconv_block(M_s[k], M_s[k+1]))

        self.h_s = nn.Sequential(*layers)
...
class LoC_LIC_Lite(CompressionModel):
    r"""
    Implementation of the CompressAI package.
    ...
    def __init__(self, config, **kwargs):
        ...
        for k in range(len(N)):
            if k == 0:
                layers.append(conv_block(3, N[k], stride=2))
            else:
                layers.append(conv_block(N[k-1], N[k], stride=2))
        layers.append(conv_block(N[-1], M[0], stride=2))
        self.g_a = nn.Sequential(*layers)
        ...
        for k in range(len(N_s)):
            if k == 0:
                layers.append(deconv_block(M[0], N_s[k], stride=2))
            else:
                layers.append(deconv_block(N_s[k-1], N_s[k], stride=2))
        layers.append(deconv_block(N_s[k], 3, stride=2))
        self.g_s = nn.Sequential(*layers)
...
class LoC_LIC(CompressionModel):
    '''
    **LoC-LIC**: Low Complexity Learned Image Codec, 
    Based MLIC++(2024) implementation https://github.com/JiangWeibeta/MLIC
    '''
    def __init__(self, config, **kwargs):
        super().__init__(config.N[-1], **kwargs)
        N = config.N
        assert len(config.M)==1, "Only single M value is supported for now"
        M = config.M[0]
        ...
        self.h_a = HyperAnalysis(M=M, N=N[-1])
        self.h_s = HyperSynthesis(M=M, N=N[-1])
